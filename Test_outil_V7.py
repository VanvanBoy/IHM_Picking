...
class StockApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        close_button = tk.Button(self, text="Fermer l'appli", command=self.close_app, bg="#FF6961", fg="white")
        ...
        if conn:
            try:
                ...
                if result:
                    ...
                    query_plateau = "SELECT id_plateau FROM plateaux WHERE id = %s"
    ...
    def switch_plateau(self):
        ...
        if conn:
            try:
                ...
                query_check = "SELECT code_emplacement FROM emplacements WHERE numero_serie = %s"
                ...
                query_check_new = "SELECT est_occupe FROM emplacements WHERE code_emplacement = %s"
    ...
    def put_in_plateau(self):
        ...
        if conn:
            try:
                ...
                query_type_plateau = "SELECT type_plateau FROM plateaux WHERE id_plateau = %s"
                ...
                query_plateau = "SELECT id FROM plateaux WHERE id_plateau = %s"
                ...
                if len(list_emplacement) < len(list_cell):
                    messagebox.showerror("Erreur", "Pas assez d'emplacements disponibles pour toutes les cellules.")
                    return
    ...
    def setup_picking(self, frame):
        ...
        self.tree_picking_module.heading("numero_serie", text="Num Série")
        self.tree_picking_module.heading("module", text="Module")
        ...
        self.tree_picking_module.pack(side="left", fill="both", expand=True)
    ...
    def load_current_cell(self):
        if self.current_index >= len(self.picking_data):
            self.finish_picking()
            return
        ...
        if conn:
            try:
                ...
                query = """
                    ...
                    FROM emplacements e 
                    JOIN plateaux p ON e.plateau_id = p.id
