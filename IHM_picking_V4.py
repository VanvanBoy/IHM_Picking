# -*- coding: utf-8 -*-
"""
Created on Mon Jul 28 12:36:43 2025
...
class StockApp(tk.Tk):
    ...
    def update_cell_state(self, numero_serie, index, state):
        color_map = {
            "none": "white",
            "picked": "lime green",
            "missing": "red"
        ...
        # Update dans la grille graphique
        try:
            i, j = self.get_battery_grid_coords(index)
            canvas, rect = self.battery_canvases[(i, j)]
            canvas.itemconfig(rect, fill=color_map[state])
            self.battery_cell_states[(i, j)] = state
        except Exception as e:
            print(f"Erreur coordonnées cellule: {e}")
    
        # Update dans TreeView 2
        if numero_serie in self.t2_item_map:
            ...
            if state == "picked":
                ...
                self.tree_picking_module.item(item_id, tags=())
    ...
    def refresh_battery_grid_from_picking(self):
        ...
        # Récupérer l'identifiant de la batterie courante
            ...
            try:
                if mode == "Picking":
                    ...
                    if self.current_index > 0:
                        ...
                        texte = self.label_info_1.cget("text")
                        reference = texte.split(": ", 1)[-1]
                        
                        # Vider l'ancienne grille
                        for widget in self.battery_grid_frame.winfo_children():
                            widget.destroy()
                        
                        # Recréer la bonne grille
                        if reference == "EMBR036AA":
    ...
    def load_current_cell(self):
        ...
        while self.current_index < len(self.picking_data):
            ...
            if current_row in self.missed_cells:
                self.update_cell_state(numero_serie, self.current_index, "missing")
    ...
    def refresh_module_tree(self, module, produit):
        ...
        for row in self.picking_data:
            ...
            if p == produit : #and etat == 0: #m == module and
                ...
                self.tree_picking_module.item(item_id, tags=())  # reset

                if etat == 1:
                    self.tree_picking_module.item(item_id, tags=("found",))
                elif etat == 0 and num in [row["Numero_serie_cellule"] for row in self.missed_cells]:
                    self.tree_picking_module.item(item_id, tags=("not_found",))
    ...
    def validate_cell_scan(self, scanned_number):
        ...
        if scanned_number == expected_num_serie:
            ...
            #self.battery_cell_states[(i, j)] = "picked"
            self.update_cell_state(scanned_number, self.current_index, "picked")
